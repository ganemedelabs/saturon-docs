...
- **`options`**: An optional object configuring the output format:
    ...
    - **`fit`**: String (default `"clip"`). Specifies the gamut mapping method (`"clip"`, `"chroma-reduction"`, `"css-gamut-map"`) for out-of-gamut colors. Can be changed in the default configuration via `configure` (see [API: config](./api/config)).
...
The `with()` method creates a new `Color` instance with updated or replaced component values in the current model. It supports direct component updates, functional updates, bulk updates via functions, and direct or functional coordinate array replacements. It also provides optional normalization behavior for handling inappropriate numeric input values while keeping all valid real numbers unchanged.
...
- **`values`**:
    - **Object**: Partial component mapping (numbers or updater functions).
    - **Function**: Receives current component values; returns a partial object or coordinate array.
    - **Array**: Replaces component coordinates directly.
- **`normalized`** (default `true`):
  Controls the handling of inappropriate numeric values:
    - `NaN` → becomes `0`
    - `Infinity` → becomes component `max`
    - `-Infinity` → becomes component `min`
      When `normalized` is `false`, these inappropriate values are preserved.
- **Returns**: A new `Color` instance with updated coordinates.
- **Throws**: If the current model has no component definitions.

### Behavior

- Component definitions are taken from `colorModels[model]`, with `alpha` added as a fourth component.
- Current coordinates are read before applying updates.
- If `values` is a function, it is called with current component values and its result is processed like object or array input.
- If `values` is an array, each index replaces the corresponding coordinate, using the normalization rules only for inappropriate values; valid numbers remain unchanged.
- If `values` is an object, each specified component is updated directly or via an updater function, again normalizing only inappropriate numbers.
- Alpha is preserved unless explicitly replaced, and a new `Color` instance is returned with the updated components.
...
const rgb = Color.from("rgb(255 87 51)");
const updated = rgb.with({ g: 128 });
console.log(updated.toString()); // rgb(255 128 51)

// Functional component update
const dimmed = rgb.with({ r: (r) => r * 0.8 });
console.log(dimmed.toString()); // rgb(204 87 51)

// Functional bulk update
const hsl = Color.from("hsl(50 100% 30%)");
const adjusted = hsl.with(({ h, s, l }) => ({
    h: h + 10,
    s: s * (l / 100),
}));
console.log(adjusted.toString()); // hsl(60 30 30)

// Direct coordinate array replacement
const newRgb = rgb.with([, 150, 200]);
console.log(newRgb.toString()); // rgb(255 150 200)

// Functional coordinate array update
const muted = rgb.with(({ r, g, b }) => [r * 0.5, g * 0.5, b * 0.5, 1]);
console.log(muted.toString()); // rgb(128 44 26)

// Disabling normalization preserves inappropriate values
const edge = rgb.with({ r: (r) => isNaN(r) ? 0 + 10 ? r + 10 }, false);
console.log(edge.toString()); // rgb(10 200 0)
```
...
- Handles hue interpolation for components marked as `hue` (e.g., `h` in `hsl`) using the specified `hue` method (`shorter`, `longer`, `increasing`, `decreasing`).
...
## Instance `Color.fit()` [#fit]

The `fit()` method fits the current `Color` instance into its own model's gamut, returning a new `Color` with component values adjusted according to the chosen fitting method. It ensures that all component values fall within the valid range of the current model (e.g., clamping overly wide-gamut values in `rgb`, restricting chroma in `oklch`, etc.). This is useful when sanitizing colors after transformations or enforcing compatibility with target rendering environments.
...
- **`options`**: Optional configuration object:
    - **`fit`**: The gamut-fitting method to apply. Defaults to `config.defaults.fit`. Supported values include:
        - `"clip"`: Clamps component values to their valid ranges.
        - `"chroma-reduction"`: Reduces chroma while maintaining hue and lightness structure.
        - `"css-gamut-map"`: Uses the CSS Color Module Level 4 gamut-mapping algorithm.
    - **`precision`**: Optional numeric precision for rounding component values during fitting.
- **Returns**: A new `Color` instance in the current model with fitted component values.
- **Throws**: Never throws unless downstream conversion rules throw.

### Behavior

- Extracts the fitting method and precision from the `options` object, using defaults when omitted.
- Calls `toArray({ fit: method, precision })` to obtain fitted component values in the current model.
- Returns a new `Color` instance constructed with the fitted coordinates, preserving the same model.
...
// Fit an out-of-range color to the RGB's target gamut, which is sRGB
const c = new Color("rgb", [300, -20, 500]);
const fitted = c.fit({ fit: "clip" });
console.log(fitted.to("rgb")); // rgb(255 0 255)
```
...
- **`epsilon`**: Optional tolerance (default to the value of `EPSILON` in `"saturon/math"` which is `1e-5`). Allows small floating-point differences.
...
- **`epsilon`**: Optional tolerance (defaults to the value of `EPSILON` in `"saturon/math"` which is `1e-5`) for boundary checks.
